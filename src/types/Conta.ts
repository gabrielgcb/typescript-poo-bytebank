...
import { ValidaTransacao } from "../utils/Decorators.js";
...
export class Conta {
    ...
    @ValidaTransacao("debito")
    private debitar(valor: number): void {
        this.saldo -= valor;
        Armazenador.salvar("saldo", this.saldo.toString());
    }
    
    @ValidaTransacao("deposito")
...
export class ContaPremium extends Conta {
    registrarTransacao(transacao: Transacao) : void {
        if(transacao.tipoTransacao === TipoTransacao.DEPOSITO) {
            console.log("Ganhou R$ 0,50 centavos!");
            transacao.valor += 0.5;
        }
...
const conta = new Conta("Gabrel");

const contaPremium = new ContaPremium("Gabriel");
...
contaPremium.registrarTransacao(transacao);
